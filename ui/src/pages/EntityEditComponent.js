...
checkEntityName() {
 ...
 // let entities = this.props.main.entities.filter(entity => entity.entityName !== this.props.editComponent.entityName)
    ...
    revertEntity() {
      ...
      this.props.showCardToggle()
      this.props.hideNewCard()
      this.props.actions.clearFieldsInEditComponent()
    ...
    let disabled = (editComponent.entityName.length === 0 || entityNameError.length > 0);
		return (
			<div>
...
EntityEditComponent.propTypes = {
    ...
    showCardToggle: PropTypes.func.isRequired
}

const selector = createSelector(
  state => state.entityScreen.main,
  state => state.entityScreen.editComponent,
  (main, editComponent) => ({ main, editComponent })
)

export default connect(selector, bindDispatch)(EntityEditComponent);
